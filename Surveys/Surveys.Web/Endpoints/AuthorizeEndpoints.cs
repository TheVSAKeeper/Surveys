...
public sealed class AuthorizeEndpoints : AppDefinition
{
        ...
        IOpenIddictAuthorizationManager authorizationManager)
    {
        ...
        OpenIddictRequest iddictRequest = httpContext.GetOpenIddictServerRequest() ?? throw new InvalidOperationException("The OpenID Connect request cannot be retrieved.");
        ...
        List<object> authorizations = await authorizationManager.FindAsync(userId,
                applicationId!,
                OpenIddictConstants.Statuses.Valid,
                OpenIddictConstants.AuthorizationTypes.Permanent,
                iddictRequest.GetScopes())
            .ToListAsync();
        ...
        switch (await applicationManager.GetConsentTypeAsync(application))
        {
            ...
            case OpenIddictConstants.ConsentTypes.External when authorizations.Count != 0:
            case OpenIddictConstants.ConsentTypes.Explicit when authorizations.Count != 0 && iddictRequest.HasPrompt(OpenIddictConstants.Prompts.Consent) == false:
                ...
                object authorization = authorizations.LastOrDefault()
                                       ?? await authorizationManager.CreateAsync(principal,
