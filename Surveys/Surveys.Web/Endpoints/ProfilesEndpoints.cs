...
using Surveys.Web.Application.Messaging.ProfileMessages.Queries;
...
internal static class ProfilesEndpointDefinitionExtensions
{
    ...
    {
        RouteGroupBuilder group = routes.MapGroup("/api/profiles").WithTags("Profiles");

        group.MapGet("roles", async ([FromServices] IMediator mediator, HttpContext context)
                => await mediator.Send(new GetRoles.Request(), context.RequestAborted))
